  ...
  <table width="100%" cellpadding="0" cellspacing="0" style="background:#fdf8f4;padding:32px 16px;">
    <tr><td align="center">
      <table width="600" cellpadding="0" cellspacing="0" style="max-width:600px;width:100%;background:#ffffff;border-radius:16px;overflow:hidden;box-shadow:0 4px 24px rgba(90,60,30,0.08);">
        ...
        <tr><td style="background:linear-gradient(135deg,#8b5e3c 0%,#a0522d 100%);padding:36px 40px;text-align:center;">
          <p style="margin:0 0 6px;color:rgba(255,255,255,0.7);font-size:13px;letter-spacing:0.08em;text-transform:uppercase;">Aavafloristi</p>
          <h1 style="margin:0;color:#ffffff;font-size:26px;font-weight:700;">Varausvahvistus</h1>
          <p style="margin:10px 0 0;color:rgba(255,255,255,0.85);font-size:14px;">Kiitos varauksestasi, ${data.customerName}! 💆</p>
        </td></tr>

        <tr><td style="padding:24px 40px 0;">
          <h2 style="margin:0 0 12px;font-size:15px;font-weight:600;color:#3d2c1e;">Varauksen tiedot</h2>
          <div style="background:#fdf8f4;border:1px solid #ecddd3;border-radius:10px;padding:16px 20px;">
            <p style="margin:0 0 6px;color:#3d2c1e;font-weight:600;">${data.service}</p>
            <p style="margin:4px 0;color:#3d2c1e;">🕐 ${fmtDate(data.date)} klo ${data.time}</p>
            <p style="margin:4px 0;color:#3d2c1e;">⏱️ ${data.duration} min</p>
            <p style="margin:4px 0;color:#8b5e3c;font-weight:700;">${fmt(data.servicePrice)}</p>
            ${data.notes ? `<p style="margin:8px 0 0;color:#9e8575;font-size:13px;">📝 ${data.notes}</p>` : ''}
          </div>
        </td></tr>
...
function buildAdminBookingNotificationHtml(data: BookingEmailData): string {
  return `<!DOCTYPE html>
<html lang="fi">
<head><meta charset="UTF-8"></head>
<body style="margin:0;padding:0;background:#f5f5f5;font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',sans-serif;">
    ...
    <tr><td align="center">
      <table width="600" cellpadding="0" cellspacing="0" style="max-width:600px;width:100%;background:#ffffff;border-radius:12px;overflow:hidden;box-shadow:0 2px 12px rgba(0,0,0,0.08);">
        ...
        <tr><td style="background:#1a1a2e;padding:24px 32px;">
          <h1 style="margin:0;color:#ffffff;font-size:20px;font-weight:700;">💆 Uusi varaus</h1>
          <p style="margin:6px 0 0;color:#aaa;font-size:13px;">Aavafloristi Admin</p>
        </td></tr>

        <tr><td style="padding:20px 32px 0;">
          <div style="background:#fef3c7;border:1px solid #f59e0b;border-radius:8px;padding:14px 18px;">
            <p style="margin:0;font-size:14px;color:#92400e;">⚡ <strong>Uusi varaus saapunut</strong> — tarkista adminissa.</p>
          </div>
        </td></tr>

        <tr><td style="padding:20px 32px 0;">
          <h2 style="margin:0 0 10px;font-size:13px;font-weight:600;color:#888;text-transform:uppercase;letter-spacing:0.06em;">Palvelu</h2>
          <div style="border:1px solid #e5e7eb;border-radius:8px;padding:14px 18px;">
            <p style="margin:0 0 4px;font-weight:600;color:#1a1a2e;">${data.service}</p>
            <p style="margin:2px 0;color:#555;">🕐 ${fmtDate(data.date)} klo ${data.time} · ⏱️ ${data.duration} min</p>
            <p style="margin:6px 0 0;font-weight:700;color:#059669;">${fmt(data.servicePrice)}</p>
          </div>
        </td></tr>
        ...
        <tr><td style="padding:20px 32px 0;">
          ...
          <div style="border:1px solid #e5e7eb;border-radius:8px;padding:14px 18px;">
            ...
            <p style="margin:2px 0;color:#555;">${data.customerPhone}</p>
            <p style="margin:2px 0;color:#555;">${data.customerEmail}</p>
            ${data.notes ? `<p style="margin:8px 0 0;color:#555;font-size:13px;">📝 ${data.notes}</p>` : ''}
          </div>
        </td></tr>
...
export async function sendBookingConfirmationEmail(data: BookingEmailData): Promise<void> {
  try {
    ...
    const { data: result, error } = await resend.emails.send({
      from: FROM_ADDRESS,
      to: data.customerEmail,
      subject: `Varausvahvistus – ${data.service}`,
      html: buildBookingConfirmationHtml(data),
    });

    if (error) {
      console.error('[EMAIL] Resend error (booking confirmation):', JSON.stringify(error));
    } else {
      console.log('[EMAIL] Booking confirmation sent:', result?.id, '→', data.customerEmail);
    }
  } catch (err) {
...
export async function sendAdminBookingNotification(data: BookingEmailData): Promise<void> {
  const adminEmail = process.env.ADMIN_EMAIL;
  if (!adminEmail) {
    console.warn('[EMAIL] ADMIN_EMAIL not set — skipping admin booking notification');
    return;
  }
  ...
  try {
    ...
    const { data: result, error } = await resend.emails.send({
      from: FROM_ADDRESS,
      to: adminEmail,
      subject: `💆 Uusi varaus — ${data.customerName}`,
      html: buildAdminBookingNotificationHtml(data),
    });

    if (error) {
      console.error('[EMAIL] Resend error (admin booking notification):', JSON.stringify(error));
    } else {
      console.log('[EMAIL] Admin booking notification sent:', result?.id, '→', adminEmail);
    }
  } catch (err) {
    console.error('[EMAIL] sendAdminBookingNotification failed:', err);
  }
}
