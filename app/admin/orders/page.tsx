        ...
        <div className="lg:hidden divide-y divide-stone-100">
          ...
          {!loading && orders.map((o) => (
            <Link
              ...
              className="block px-4 py-3.5 hover:bg-stone-50 transition-colors"
            >
              <div className="flex items-start justify-between gap-3 mb-2">
                <div className="min-w-0">
                  <p className="font-medium text-stone-800 truncate">{o.customerName}</p>
                  <p className="font-mono text-xs text-stone-400 truncate">{o.orderNumber.slice(0, 18)}…</p>
                </div>
                <OrderStatusBadge status={o.status} />
              </div>
              <div className="flex items-center justify-between gap-3 text-xs text-stone-500">
                <span className="truncate">{o.customerPhone}</span>
                <span className="font-semibold text-stone-800 whitespace-nowrap">{fmt(o.total)}</span>
              </div>
              <div className="flex items-center justify-between gap-3 text-xs text-stone-400 mt-1">
                <span className="truncate">{DELIVERY_LABELS[lang][o.deliveryType] || o.deliveryType}</span>
                <span className="whitespace-nowrap">{fmtDate(o.createdAt)}</span>
              </div>
            </Link>
          ))}
        </div>
